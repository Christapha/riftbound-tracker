...
Everything about running, using, and publishing the tracker. For a short overview see the
[README](../README.md).

Nothing here leaves your machine unless you explicitly publish a snapshot.
...
## The visitor want list

The published site lets people build a list from what you own and send it to you. Each
card shows how many you have and a stepper to request some of them; **Your list** in the
top bar opens the basket.

Requests are **capped at your stock** — nobody can ask for three copies of something you
have one of. The list is re-checked against the current snapshot every time it opens, so
items you've since sold or removed drop out with a note rather than lingering.

Three ways to send it:

- **Copy list** — plain text, one line per card with set, number, finish and reference
  price. Fine for Discord, email or a message.
- **Copy link** — the same list encoded in the URL. You open it and see exactly their
  basket, no parsing.
- **Download .txt** — for when the browser blocks clipboard access.

Set the **contact line** when you publish (Data → Publish) and it appears in the panel so
visitors know where to send things. Leave it blank and the wording falls back to something
generic.

Worth being clear with people about, since the interface can imply otherwise: nothing here
reserves a card, nothing notifies you, and the prices are TCGplayer market reference rather
than your asking price. The panel says so, but it's the sort of thing worth repeating when
someone sends their first list. A basket sat in someone's browser for a week is not a
claim on anything.
...
src/lib/catalog.js           loading, normalization, set tables, sorting, local API calls
